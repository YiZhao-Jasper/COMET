"""
COMET Utils — Model building and sampling utilities
=====================================================

Based on MOTFM/utils/utils_fm.py, modified to support warm-start coupling
during inference. Changes marked with [COMET].
"""
import os
import json
import torch
...
# [COMET] Import warm-start utility
from losses.contrastive_flow import create_warm_start_noise
...
def validate_and_save_samples(
    ...
    warm_start_config=None,  # [COMET] warm-start params dict
):
    model.eval()
    ...
    for batch in tqdm(val_loader, desc="Validating"):
        ...
        # [COMET] Warm-start coupling for inference
        # Adapted from ConditionalPriorFM: start from condition-specific prior
        noise = torch.randn_like(imgs)
        if warm_start_config is not None and masks is not None:
            x_init = create_warm_start_noise(
                masks=masks,
                noise=noise,
                alpha=float(warm_start_config.get("warm_start_alpha", 0.3)),
                blur_kernel_size=int(warm_start_config.get("warm_start_blur_kernel", 15)),
                blur_sigma=float(warm_start_config.get("warm_start_blur_sigma", 5.0)),
            )
        else:
            x_init = noise
